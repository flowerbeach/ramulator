...
namespace ramulator {
    ...
    private:
        ...
        typename T::Command get_first_cmd(Request &req) {
            ...
            switch (int(req.type)) {
                ...
                case int(Request::Type::WRITE): {
                    ...
                    else if (node->row_state.find(req.addr_vec[int(T::Level::Row)]) != node->row_state.end())
                        return cmd;
        ...
        void
        update(typename T::Command cmd, bool state_change, vector<int>::iterator &begin, vector<int>::iterator &end,
               request_queue &q) {
            if (q.empty()) return;
